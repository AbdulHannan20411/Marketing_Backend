// <auto-generated />
using System;
using System.Collections.Generic;
using Marketing.DataAccess.Context;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;

#nullable disable

namespace Marketing.DataAccess.Migrations
{
    [DbContext(typeof(ApplicationDbContext))]
    [Migration("20260925183233_AddNotificationDismissals")]
    partial class AddNotificationDismissals
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "10.0.10")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("Marketing.DataAccess.Entities.ActivityEntry", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<string>("Action")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("action");

                    b.Property<string>("Actor")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)")
                        .HasColumnName("actor");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<DateTimeOffset>("OccurredOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("occurred_on");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<string>("Subject")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("subject");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.HasKey("Id")
                        .HasName("pk_activity_entries");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_activity_entries_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_activity_entries_tenant_id_id");

                    b.HasIndex("TenantId", "OccurredOn")
                        .IsDescending(false, true)
                        .HasDatabaseName("ix_activity_entries_tenant_id_occurred_on");

                    b.ToTable("activity_entries", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.AuditLog", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<string>("Action")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("action");

                    b.Property<string>("Changes")
                        .HasColumnType("jsonb")
                        .HasColumnName("changes");

                    b.Property<string>("CorrelationId")
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("correlation_id");

                    b.Property<string>("EntityId")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("entity_id");

                    b.Property<string>("EntityName")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("entity_name");

                    b.Property<string>("IpAddress")
                        .HasMaxLength(45)
                        .HasColumnType("character varying(45)")
                        .HasColumnName("ip_address");

                    b.Property<DateTimeOffset>("OccurredOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("occurred_on");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.Property<long>("UserId")
                        .HasColumnType("bigint")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("pk_audit_logs");

                    b.HasIndex("UserId")
                        .HasDatabaseName("ix_audit_logs_user_id");

                    b.HasIndex("TenantId", "OccurredOn")
                        .IsDescending(false, true)
                        .HasDatabaseName("ix_audit_logs_tenant_id_occurred_on");

                    b.HasIndex("EntityName", "EntityId", "OccurredOn")
                        .IsDescending(false, false, true)
                        .HasDatabaseName("ix_audit_logs_entity_name_entity_id_occurred_on");

                    b.ToTable("audit_logs", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.AutoReplyAttempt", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<DateTimeOffset>("AttemptedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("attempted_at");

                    b.Property<long>("ConversationId")
                        .HasColumnType("bigint")
                        .HasColumnName("conversation_id");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<bool>("FallbackSent")
                        .HasColumnType("boolean")
                        .HasColumnName("fallback_sent");

                    b.Property<DateTimeOffset>("InboundMessageAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("inbound_message_at");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<string>("Outcome")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("outcome");

                    b.Property<string>("Question")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("question");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.Property<string>("Trigger")
                        .IsRequired()
                        .HasMaxLength(32)
                        .HasColumnType("character varying(32)")
                        .HasColumnName("trigger");

                    b.HasKey("Id")
                        .HasName("pk_auto_reply_attempts");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_auto_reply_attempts_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("ConversationId", "InboundMessageAt")
                        .HasDatabaseName("ix_auto_reply_attempts_conversation_id_inbound_message_at");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_auto_reply_attempts_tenant_id_id");

                    b.HasIndex("TenantId", "Outcome", "AttemptedAt")
                        .HasDatabaseName("ix_auto_reply_attempts_tenant_id_outcome_attempted_at");

                    b.ToTable("auto_reply_attempts", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.AutoReplyKnowledgeEntry", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<string>("Answer")
                        .IsRequired()
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)")
                        .HasColumnName("answer");

                    b.Property<bool?>("Available")
                        .HasColumnType("boolean")
                        .HasColumnName("available");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.PrimitiveCollection<List<string>>("Keywords")
                        .IsRequired()
                        .HasColumnType("text[]")
                        .HasColumnName("keywords");

                    b.Property<string>("Kind")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("kind");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<string>("Price")
                        .HasMaxLength(60)
                        .HasColumnType("character varying(60)")
                        .HasColumnName("price");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<int>("SortOrder")
                        .HasColumnType("integer")
                        .HasColumnName("sort_order");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("title");

                    b.HasKey("Id")
                        .HasName("pk_auto_reply_knowledge_entries");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_auto_reply_knowledge_entries_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_auto_reply_knowledge_entries_tenant_id_id");

                    b.HasIndex("TenantId", "SortOrder")
                        .HasDatabaseName("ix_auto_reply_knowledge_entries_tenant_id_sort_order");

                    b.ToTable("auto_reply_knowledge_entries", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.AutoReplySettings", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<int>("DelaySeconds")
                        .HasColumnType("integer")
                        .HasColumnName("delay_seconds");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<bool>("Enabled")
                        .HasColumnType("boolean")
                        .HasColumnName("enabled");

                    b.Property<bool>("FirstMessageEnabled")
                        .HasColumnType("boolean")
                        .HasColumnName("first_message_enabled");

                    b.Property<bool>("GreetingEnabled")
                        .HasColumnType("boolean")
                        .HasColumnName("greeting_enabled");

                    b.Property<string>("Instructions")
                        .IsRequired()
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)")
                        .HasColumnName("instructions");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<string>("KnowledgeFallback")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasDefaultValue("handoff")
                        .HasColumnName("knowledge_fallback");

                    b.Property<string>("KnowledgeFallbackMessage")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(300)
                        .HasColumnType("character varying(300)")
                        .HasDefaultValue("Thanks for your message! Someone from our team will get back to you shortly.")
                        .HasColumnName("knowledge_fallback_message");

                    b.Property<string>("KnowledgeSourceFileName")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("knowledge_source_file_name");

                    b.Property<DateTimeOffset?>("KnowledgeUpdatedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("knowledge_updated_at");

                    b.Property<long?>("KnowledgeUpdatedByUserId")
                        .HasColumnType("bigint")
                        .HasColumnName("knowledge_updated_by_user_id");

                    b.Property<int>("MaxPerConversationPerDay")
                        .HasColumnType("integer")
                        .HasColumnName("max_per_conversation_per_day");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<DateTimeOffset?>("QuotaNoticeSentForPeriodEnd")
                        .HasColumnType("timestamptz")
                        .HasColumnName("quota_notice_sent_for_period_end");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.Property<int>("UnansweredAfterMinutes")
                        .HasColumnType("integer")
                        .HasColumnName("unanswered_after_minutes");

                    b.Property<bool>("UnansweredEnabled")
                        .HasColumnType("boolean")
                        .HasColumnName("unanswered_enabled");

                    b.HasKey("Id")
                        .HasName("pk_auto_reply_settings");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_auto_reply_settings_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("TenantId")
                        .IsUnique()
                        .HasDatabaseName("ix_auto_reply_settings_tenant_id")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_auto_reply_settings_tenant_id_id");

                    b.ToTable("auto_reply_settings", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.BillingProfile", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<string>("AddressLine1")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("address_line1");

                    b.Property<string>("AddressLine2")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("address_line2");

                    b.Property<string>("BillingEmail")
                        .IsRequired()
                        .HasMaxLength(254)
                        .HasColumnType("character varying(254)")
                        .HasColumnName("billing_email");

                    b.Property<string>("City")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("city");

                    b.Property<string>("CompanyName")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("company_name");

                    b.Property<string>("Country")
                        .IsRequired()
                        .HasMaxLength(2)
                        .HasColumnType("character varying(2)")
                        .HasColumnName("country");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<string>("PostalCode")
                        .IsRequired()
                        .HasMaxLength(24)
                        .HasColumnType("character varying(24)")
                        .HasColumnName("postal_code");

                    b.Property<string>("Region")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("region");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<string>("TaxId")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("tax_id");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.HasKey("Id")
                        .HasName("pk_billing_profiles");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_billing_profiles_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("TenantId")
                        .IsUnique()
                        .HasDatabaseName("ix_billing_profiles_tenant_id")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_billing_profiles_tenant_id_id");

                    b.ToTable("billing_profiles", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.Campaign", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<long?>("ActiveCampaignRunId")
                        .HasColumnType("bigint")
                        .HasColumnName("active_campaign_run_id");

                    b.PrimitiveCollection<List<long>>("AudienceGroupIds")
                        .IsRequired()
                        .HasColumnType("bigint[]")
                        .HasColumnName("audience_group_ids");

                    b.Property<string>("AudienceLabel")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("audience_label");

                    b.Property<int>("AudienceSize")
                        .HasColumnType("integer")
                        .HasColumnName("audience_size");

                    b.Property<int>("ClickedCount")
                        .HasColumnType("integer")
                        .HasColumnName("clicked_count");

                    b.Property<DateTimeOffset?>("CompletedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("completed_at");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<string>("CreatedByName")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)")
                        .HasColumnName("created_by_name");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<int>("DeliveredCount")
                        .HasColumnType("integer")
                        .HasColumnName("delivered_count");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)")
                        .HasColumnName("description");

                    b.Property<int>("FailedCount")
                        .HasColumnType("integer")
                        .HasColumnName("failed_count");

                    b.Property<long?>("HeaderMediaId")
                        .HasColumnType("bigint")
                        .HasColumnName("header_media_id");

                    b.Property<string>("HeaderMetaMediaId")
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("header_meta_media_id");

                    b.Property<string>("HeaderMetaMediaPhoneNumberId")
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("header_meta_media_phone_number_id");

                    b.Property<DateTimeOffset?>("HeaderMetaMediaUploadedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("header_meta_media_uploaded_at");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<DateTimeOffset?>("LastRunAtUtc")
                        .HasColumnType("timestamptz")
                        .HasColumnName("last_run_at_utc");

                    b.Property<long?>("MessageTemplateId")
                        .HasColumnType("bigint")
                        .HasColumnName("message_template_id");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("name");

                    b.Property<DateTimeOffset?>("NextRunAtUtc")
                        .HasColumnType("timestamptz")
                        .HasColumnName("next_run_at_utc");

                    b.Property<int>("OccurrencesRun")
                        .HasColumnType("integer")
                        .HasColumnName("occurrences_run");

                    b.Property<int>("ReadCount")
                        .HasColumnType("integer")
                        .HasColumnName("read_count");

                    b.Property<DateTimeOffset?>("RecipientsQueuedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("recipients_queued_on");

                    b.Property<string>("RecurrenceJson")
                        .HasColumnType("jsonb")
                        .HasColumnName("recurrence_json");

                    b.Property<DateTimeOffset?>("ResumeAfter")
                        .HasColumnType("timestamptz")
                        .HasColumnName("resume_after");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<DateTimeOffset?>("ScheduledAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("scheduled_at");

                    b.Property<int>("SentCount")
                        .HasColumnType("integer")
                        .HasColumnName("sent_count");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("status");

                    b.Property<string>("TemplateName")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("template_name");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.Property<string>("TimeZone")
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("time_zone");

                    b.Property<long?>("WhatsAppConnectionId")
                        .HasColumnType("bigint")
                        .HasColumnName("whats_app_connection_id");

                    b.HasKey("Id")
                        .HasName("pk_campaigns");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_campaigns_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("MessageTemplateId")
                        .HasDatabaseName("ix_campaigns_message_template_id");

                    b.HasIndex("NextRunAtUtc")
                        .HasDatabaseName("ix_campaigns_next_run_at_utc")
                        .HasFilter("next_run_at_utc IS NOT NULL AND is_deleted = false");

                    b.HasIndex("Status", "ScheduledAt")
                        .HasDatabaseName("ix_campaigns_status_scheduled_at");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_campaigns_tenant_id_id");

                    b.HasIndex("TenantId", "WhatsAppConnectionId")
                        .HasDatabaseName("ix_campaigns_tenant_id_whats_app_connection_id");

                    b.HasIndex("TenantId", "Status", "CreatedOn")
                        .HasDatabaseName("ix_campaigns_tenant_id_status_created_on");

                    b.ToTable("campaigns", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.CampaignMessage", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<int>("AttemptCount")
                        .HasColumnType("integer")
                        .HasColumnName("attempt_count");

                    b.Property<long>("CampaignId")
                        .HasColumnType("bigint")
                        .HasColumnName("campaign_id");

                    b.Property<long?>("CampaignRunId")
                        .HasColumnType("bigint")
                        .HasColumnName("campaign_run_id");

                    b.Property<long>("ContactId")
                        .HasColumnType("bigint")
                        .HasColumnName("contact_id");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<DateTimeOffset?>("DeliveredOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("delivered_on");

                    b.Property<int?>("ErrorCode")
                        .HasColumnType("integer")
                        .HasColumnName("error_code");

                    b.Property<string>("ErrorReason")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("error_reason");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<string>("MetaMessageId")
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("meta_message_id");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<string>("PhoneNumber")
                        .IsRequired()
                        .HasMaxLength(32)
                        .HasColumnType("character varying(32)")
                        .HasColumnName("phone_number");

                    b.Property<DateTimeOffset?>("ReadOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("read_on");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<DateTimeOffset?>("SentOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("sent_on");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("status");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.HasKey("Id")
                        .HasName("pk_campaign_messages");

                    b.HasIndex("ContactId")
                        .HasDatabaseName("ix_campaign_messages_contact_id");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_campaign_messages_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("MetaMessageId")
                        .HasDatabaseName("ix_campaign_messages_meta_message_id");

                    b.HasIndex("CampaignId", "ContactId")
                        .IsUnique()
                        .HasDatabaseName("ix_campaign_messages_campaign_id_contact_id")
                        .HasFilter("campaign_run_id IS NULL AND is_deleted = false");

                    b.HasIndex("CampaignId", "Status")
                        .HasDatabaseName("ix_campaign_messages_campaign_id_status");

                    b.HasIndex("CampaignRunId", "ContactId")
                        .IsUnique()
                        .HasDatabaseName("ix_campaign_messages_campaign_run_id_contact_id")
                        .HasFilter("campaign_run_id IS NOT NULL AND is_deleted = false");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_campaign_messages_tenant_id_id");

                    b.ToTable("campaign_messages", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.CampaignRun", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<int>("AudienceSize")
                        .HasColumnType("integer")
                        .HasColumnName("audience_size");

                    b.Property<long>("CampaignId")
                        .HasColumnType("bigint")
                        .HasColumnName("campaign_id");

                    b.Property<int>("ClickedCount")
                        .HasColumnType("integer")
                        .HasColumnName("clicked_count");

                    b.Property<DateTimeOffset?>("CompletedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("completed_at");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<int>("DeliveredCount")
                        .HasColumnType("integer")
                        .HasColumnName("delivered_count");

                    b.Property<int>("FailedCount")
                        .HasColumnType("integer")
                        .HasColumnName("failed_count");

                    b.Property<string>("FailureReason")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("failure_reason");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<int>("OccurrenceNumber")
                        .HasColumnType("integer")
                        .HasColumnName("occurrence_number");

                    b.Property<int>("ReadCount")
                        .HasColumnType("integer")
                        .HasColumnName("read_count");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<DateTimeOffset>("ScheduledForUtc")
                        .HasColumnType("timestamptz")
                        .HasColumnName("scheduled_for_utc");

                    b.Property<int>("SentCount")
                        .HasColumnType("integer")
                        .HasColumnName("sent_count");

                    b.Property<int>("SkippedCount")
                        .HasColumnType("integer")
                        .HasColumnName("skipped_count");

                    b.Property<DateTimeOffset?>("StartedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("started_at");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("status");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.Property<bool>("TriggeredManually")
                        .HasColumnType("boolean")
                        .HasColumnName("triggered_manually");

                    b.HasKey("Id")
                        .HasName("pk_campaign_runs");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_campaign_runs_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("CampaignId", "OccurrenceNumber")
                        .IsUnique()
                        .HasDatabaseName("ix_campaign_runs_campaign_id_occurrence_number")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("CampaignId", "ScheduledForUtc")
                        .HasDatabaseName("ix_campaign_runs_campaign_id_scheduled_for_utc");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_campaign_runs_tenant_id_id");

                    b.ToTable("campaign_runs", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.Contact", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<string>("Country")
                        .IsRequired()
                        .HasMaxLength(2)
                        .HasColumnType("character varying(2)")
                        .HasColumnName("country");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<string>("Email")
                        .HasMaxLength(320)
                        .HasColumnType("character varying(320)")
                        .HasColumnName("email");

                    b.Property<string>("FullName")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("full_name");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<DateTimeOffset?>("LastMessagedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("last_messaged_at");

                    b.Property<string>("Lifecycle")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("lifecycle");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<string>("NormalizedPhoneNumber")
                        .IsRequired()
                        .HasMaxLength(32)
                        .HasColumnType("character varying(32)")
                        .HasColumnName("normalized_phone_number");

                    b.Property<DateTimeOffset?>("OptedInAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("opted_in_at");

                    b.Property<string>("PhoneNumber")
                        .IsRequired()
                        .HasMaxLength(32)
                        .HasColumnType("character varying(32)")
                        .HasColumnName("phone_number");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("status");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.HasKey("Id")
                        .HasName("pk_contacts");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_contacts_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_contacts_tenant_id_id");

                    b.HasIndex("TenantId", "Lifecycle")
                        .HasDatabaseName("ix_contacts_tenant_id_lifecycle");

                    b.HasIndex("TenantId", "NormalizedPhoneNumber")
                        .IsUnique()
                        .HasDatabaseName("ix_contacts_tenant_id_normalized_phone_number")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("TenantId", "Status", "CreatedOn")
                        .HasDatabaseName("ix_contacts_tenant_id_status_created_on");

                    b.ToTable("contacts", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.ContactGroup", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("description");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("name");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.HasKey("Id")
                        .HasName("pk_contact_groups");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_contact_groups_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_contact_groups_tenant_id_id");

                    b.HasIndex("TenantId", "Name")
                        .IsUnique()
                        .HasDatabaseName("ix_contact_groups_tenant_id_name")
                        .HasFilter("is_deleted = false");

                    b.ToTable("contact_groups", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.ContactGroupMember", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<long>("ContactGroupId")
                        .HasColumnType("bigint")
                        .HasColumnName("contact_group_id");

                    b.Property<long>("ContactId")
                        .HasColumnType("bigint")
                        .HasColumnName("contact_id");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.HasKey("Id")
                        .HasName("pk_contact_group_members");

                    b.HasIndex("ContactId")
                        .HasDatabaseName("ix_contact_group_members_contact_id");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_contact_group_members_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("ContactGroupId", "ContactId")
                        .IsUnique()
                        .HasDatabaseName("ix_contact_group_members_contact_group_id_contact_id")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_contact_group_members_tenant_id_id");

                    b.ToTable("contact_group_members", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.ContactImportBatch", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<string>("ColumnMapping")
                        .HasColumnType("text")
                        .HasColumnName("column_mapping");

                    b.PrimitiveCollection<List<string>>("Columns")
                        .IsRequired()
                        .HasColumnType("text[]")
                        .HasColumnName("columns");

                    b.Property<DateTimeOffset?>("CommittedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("committed_on");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<int>("DuplicateRows")
                        .HasColumnType("integer")
                        .HasColumnName("duplicate_rows");

                    b.Property<int>("DuplicateStrategy")
                        .HasColumnType("integer")
                        .HasColumnName("duplicate_strategy");

                    b.Property<int>("DuplicatesInFile")
                        .HasColumnType("integer")
                        .HasColumnName("duplicates_in_file");

                    b.Property<string>("ErrorReportStorageKey")
                        .HasColumnType("text")
                        .HasColumnName("error_report_storage_key");

                    b.Property<int>("FailedCount")
                        .HasColumnType("integer")
                        .HasColumnName("failed_count");

                    b.Property<string>("FailureReason")
                        .HasColumnType("text")
                        .HasColumnName("failure_reason");

                    b.Property<string>("FileName")
                        .IsRequired()
                        .HasMaxLength(260)
                        .HasColumnType("character varying(260)")
                        .HasColumnName("file_name");

                    b.Property<long>("FileSizeBytes")
                        .HasColumnType("bigint")
                        .HasColumnName("file_size_bytes");

                    b.Property<int>("ImportedCount")
                        .HasColumnType("integer")
                        .HasColumnName("imported_count");

                    b.Property<int>("InvalidRows")
                        .HasColumnType("integer")
                        .HasColumnName("invalid_rows");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<int>("ProcessedRows")
                        .HasColumnType("integer")
                        .HasColumnName("processed_rows");

                    b.PrimitiveCollection<List<string>>("RowErrors")
                        .IsRequired()
                        .HasColumnType("text[]")
                        .HasColumnName("row_errors");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<int>("SkippedCount")
                        .HasColumnType("integer")
                        .HasColumnName("skipped_count");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(24)
                        .HasColumnType("character varying(24)")
                        .HasColumnName("status");

                    b.Property<string>("StorageKey")
                        .HasColumnType("text")
                        .HasColumnName("storage_key");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.Property<int>("TotalRows")
                        .HasColumnType("integer")
                        .HasColumnName("total_rows");

                    b.Property<int>("UpdatedCount")
                        .HasColumnType("integer")
                        .HasColumnName("updated_count");

                    b.Property<string>("UploadedByName")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("uploaded_by_name");

                    b.Property<long>("UploadedByUserId")
                        .HasColumnType("bigint")
                        .HasColumnName("uploaded_by_user_id");

                    b.Property<DateTimeOffset>("UploadedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("uploaded_on");

                    b.HasKey("Id")
                        .HasName("pk_contact_import_batches");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_contact_import_batches_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_contact_import_batches_tenant_id_id");

                    b.HasIndex("TenantId", "Status", "UploadedOn")
                        .HasDatabaseName("ix_contact_import_batches_tenant_id_status_uploaded_on");

                    b.ToTable("contact_import_batches", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.ContactImportExport", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<DateTimeOffset?>("CompletedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("completed_at");

                    b.Property<long>("ContactImportBatchId")
                        .HasColumnType("bigint")
                        .HasColumnName("contact_import_batch_id");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<string>("FailureReason")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("failure_reason");

                    b.Property<string>("FileName")
                        .IsRequired()
                        .HasMaxLength(260)
                        .HasColumnType("character varying(260)")
                        .HasColumnName("file_name");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<DateTimeOffset>("RequestedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("requested_at");

                    b.Property<long>("RequestedByUserId")
                        .HasColumnType("bigint")
                        .HasColumnName("requested_by_user_id");

                    b.Property<int>("RowCount")
                        .HasColumnType("integer")
                        .HasColumnName("row_count");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("status");

                    b.Property<string>("StorageKey")
                        .HasMaxLength(400)
                        .HasColumnType("character varying(400)")
                        .HasColumnName("storage_key");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.HasKey("Id")
                        .HasName("pk_contact_import_exports");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_contact_import_exports_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("ContactImportBatchId", "RequestedAt")
                        .HasDatabaseName("ix_contact_import_exports_contact_import_batch_id_requested_at");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_contact_import_exports_tenant_id_id");

                    b.ToTable("contact_import_exports", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.ContactImportRow", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<long>("ContactImportBatchId")
                        .HasColumnType("bigint")
                        .HasColumnName("contact_import_batch_id");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<string>("Error")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("error");

                    b.Property<string>("ErrorCode")
                        .HasMaxLength(32)
                        .HasColumnType("character varying(32)")
                        .HasColumnName("error_code");

                    b.Property<string>("ErrorField")
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("error_field");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<bool>("IsDuplicate")
                        .HasColumnType("boolean")
                        .HasColumnName("is_duplicate");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<int>("RowNumber")
                        .HasColumnType("integer")
                        .HasColumnName("row_number");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("status");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.PrimitiveCollection<List<string>>("Values")
                        .IsRequired()
                        .HasColumnType("text[]")
                        .HasColumnName("values");

                    b.HasKey("Id")
                        .HasName("pk_contact_import_rows");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_contact_import_rows_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("ContactImportBatchId", "RowNumber")
                        .HasDatabaseName("ix_contact_import_rows_contact_import_batch_id_row_number");

                    b.HasIndex("ContactImportBatchId", "Status")
                        .HasDatabaseName("ix_contact_import_rows_contact_import_batch_id_status");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_contact_import_rows_tenant_id_id");

                    b.ToTable("contact_import_rows", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.ContactTag", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<string>("Color")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("color");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(60)
                        .HasColumnType("character varying(60)")
                        .HasColumnName("name");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.HasKey("Id")
                        .HasName("pk_contact_tags");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_contact_tags_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_contact_tags_tenant_id_id");

                    b.HasIndex("TenantId", "Name")
                        .IsUnique()
                        .HasDatabaseName("ix_contact_tags_tenant_id_name")
                        .HasFilter("is_deleted = false");

                    b.ToTable("contact_tags", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.ContactTagAssignment", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<long>("ContactId")
                        .HasColumnType("bigint")
                        .HasColumnName("contact_id");

                    b.Property<long>("ContactTagId")
                        .HasColumnType("bigint")
                        .HasColumnName("contact_tag_id");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.HasKey("Id")
                        .HasName("pk_contact_tag_assignments");

                    b.HasIndex("ContactId")
                        .HasDatabaseName("ix_contact_tag_assignments_contact_id");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_contact_tag_assignments_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("ContactTagId", "ContactId")
                        .IsUnique()
                        .HasDatabaseName("ix_contact_tag_assignments_contact_tag_id_contact_id")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_contact_tag_assignments_tenant_id_id");

                    b.ToTable("contact_tag_assignments", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.Conversation", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<long?>("AssignedToUserId")
                        .HasColumnType("bigint")
                        .HasColumnName("assigned_to_user_id");

                    b.Property<long?>("ContactId")
                        .HasColumnType("bigint")
                        .HasColumnName("contact_id");

                    b.Property<string>("ContactName")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("contact_name");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<DateTimeOffset?>("LastMessageAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("last_message_at");

                    b.Property<string>("LastMessagePreview")
                        .IsRequired()
                        .HasMaxLength(300)
                        .HasColumnType("character varying(300)")
                        .HasColumnName("last_message_preview");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.Property<int>("UnreadCount")
                        .HasColumnType("integer")
                        .HasColumnName("unread_count");

                    b.Property<string>("WaId")
                        .IsRequired()
                        .HasMaxLength(32)
                        .HasColumnType("character varying(32)")
                        .HasColumnName("wa_id");

                    b.Property<long?>("WhatsAppConnectionId")
                        .HasColumnType("bigint")
                        .HasColumnName("whats_app_connection_id");

                    b.Property<DateTimeOffset?>("WindowExpiresAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("window_expires_at");

                    b.HasKey("Id")
                        .HasName("pk_conversations");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_conversations_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("TenantId", "AssignedToUserId")
                        .HasDatabaseName("ix_conversations_tenant_id_assigned_to_user_id");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_conversations_tenant_id_id");

                    b.HasIndex("TenantId", "LastMessageAt")
                        .IsDescending(false, true)
                        .HasDatabaseName("ix_conversations_tenant_id_last_message_at");

                    b.HasIndex("TenantId", "WhatsAppConnectionId", "WaId")
                        .IsUnique()
                        .HasDatabaseName("ix_conversations_tenant_id_whats_app_connection_id_wa_id")
                        .HasFilter("is_deleted = false");

                    b.ToTable("conversations", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.ConversationMessage", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<string>("Body")
                        .IsRequired()
                        .HasMaxLength(4096)
                        .HasColumnType("character varying(4096)")
                        .HasColumnName("body");

                    b.Property<long>("ConversationId")
                        .HasColumnType("bigint")
                        .HasColumnName("conversation_id");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<string>("Direction")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("direction");

                    b.Property<string>("FailureReason")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("failure_reason");

                    b.Property<bool>("IsAutoReply")
                        .HasColumnType("boolean")
                        .HasColumnName("is_auto_reply");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<string>("Kind")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("kind");

                    b.Property<long?>("MediaId")
                        .HasColumnType("bigint")
                        .HasColumnName("media_id");

                    b.Property<string>("MetaMessageId")
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("meta_message_id");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<DateTimeOffset>("OccurredAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("occurred_at");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("status");

                    b.Property<string>("TemplateName")
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("template_name");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.HasKey("Id")
                        .HasName("pk_conversation_messages");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_conversation_messages_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("MediaId")
                        .HasDatabaseName("ix_conversation_messages_media_id");

                    b.HasIndex("MetaMessageId")
                        .IsUnique()
                        .HasDatabaseName("ix_conversation_messages_meta_message_id")
                        .HasFilter("meta_message_id IS NOT NULL AND is_deleted = false");

                    b.HasIndex("ConversationId", "OccurredAt")
                        .HasDatabaseName("ix_conversation_messages_conversation_id_occurred_at");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_conversation_messages_tenant_id_id");

                    b.ToTable("conversation_messages", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.DeliveryFailure", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<long?>("CampaignId")
                        .HasColumnType("bigint")
                        .HasColumnName("campaign_id");

                    b.Property<string>("CampaignName")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("campaign_name");

                    b.Property<long?>("ContactId")
                        .HasColumnType("bigint")
                        .HasColumnName("contact_id");

                    b.Property<string>("ContactName")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("contact_name");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<int>("ErrorCode")
                        .HasColumnType("integer")
                        .HasColumnName("error_code");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<DateTimeOffset>("OccurredOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("occurred_on");

                    b.Property<string>("PhoneNumber")
                        .IsRequired()
                        .HasMaxLength(32)
                        .HasColumnType("character varying(32)")
                        .HasColumnName("phone_number");

                    b.Property<string>("Reason")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("reason");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.HasKey("Id")
                        .HasName("pk_delivery_failures");

                    b.HasIndex("CampaignId")
                        .HasDatabaseName("ix_delivery_failures_campaign_id");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_delivery_failures_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_delivery_failures_tenant_id_id");

                    b.HasIndex("TenantId", "OccurredOn")
                        .IsDescending(false, true)
                        .HasDatabaseName("ix_delivery_failures_tenant_id_occurred_on");

                    b.ToTable("delivery_failures", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.EmailTemplate", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<string>("Category")
                        .IsRequired()
                        .HasMaxLength(24)
                        .HasColumnType("character varying(24)")
                        .HasColumnName("category");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<string>("DefaultHash")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("default_hash");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(300)
                        .HasColumnType("character varying(300)")
                        .HasColumnName("description");

                    b.Property<string>("HtmlBody")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("html_body");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<string>("Key")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("key");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("name");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<string>("Subject")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("subject");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.Property<string>("TextBody")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("text_body");

                    b.Property<long?>("UpdatedByUserId")
                        .HasColumnType("bigint")
                        .HasColumnName("updated_by_user_id");

                    b.Property<DateTimeOffset?>("UpdatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("updated_on");

                    b.Property<string>("VariablesJson")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("variables_json");

                    b.HasKey("Id")
                        .HasName("pk_email_templates");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_email_templates_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("Key")
                        .IsUnique()
                        .HasDatabaseName("ix_email_templates_key")
                        .HasFilter("is_deleted = false");

                    b.ToTable("email_templates", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.ImportJob", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<int>("AttemptCount")
                        .HasColumnType("integer")
                        .HasColumnName("attempt_count");

                    b.Property<DateTimeOffset>("AvailableAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("available_at");

                    b.Property<DateTimeOffset?>("ClaimedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("claimed_at");

                    b.Property<DateTimeOffset?>("CompletedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("completed_at");

                    b.Property<long>("ContactImportBatchId")
                        .HasColumnType("bigint")
                        .HasColumnName("contact_import_batch_id");

                    b.Property<long?>("ContactImportExportId")
                        .HasColumnType("bigint")
                        .HasColumnName("contact_import_export_id");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<string>("Kind")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("kind");

                    b.Property<string>("LastError")
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)")
                        .HasColumnName("last_error");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<long>("RequestedByUserId")
                        .HasColumnType("bigint")
                        .HasColumnName("requested_by_user_id");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<string>("State")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("state");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.HasKey("Id")
                        .HasName("pk_import_jobs");

                    b.HasIndex("ContactImportBatchId")
                        .HasDatabaseName("ix_import_jobs_contact_import_batch_id");

                    b.HasIndex("ContactImportExportId")
                        .HasDatabaseName("ix_import_jobs_contact_import_export_id");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_import_jobs_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("State", "AvailableAt")
                        .HasDatabaseName("ix_import_jobs_state_available_at");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_import_jobs_tenant_id_id");

                    b.ToTable("import_jobs", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.Invoice", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<decimal>("Amount")
                        .HasPrecision(18, 4)
                        .HasColumnType("numeric(18,4)")
                        .HasColumnName("amount");

                    b.Property<string>("BillingCycle")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("billing_cycle");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<string>("Currency")
                        .IsRequired()
                        .HasMaxLength(3)
                        .HasColumnType("character varying(3)")
                        .HasColumnName("currency");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<DateTimeOffset>("DueAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("due_at");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<DateTimeOffset>("IssuedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("issued_at");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<string>("Number")
                        .IsRequired()
                        .HasMaxLength(40)
                        .HasColumnType("character varying(40)")
                        .HasColumnName("number");

                    b.Property<DateTimeOffset?>("PaidAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("paid_at");

                    b.Property<DateTimeOffset>("PeriodEnd")
                        .HasColumnType("timestamptz")
                        .HasColumnName("period_end");

                    b.Property<DateTimeOffset>("PeriodStart")
                        .HasColumnType("timestamptz")
                        .HasColumnName("period_start");

                    b.Property<string>("PlanName")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("plan_name");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("status");

                    b.Property<decimal>("Tax")
                        .HasPrecision(18, 4)
                        .HasColumnType("numeric(18,4)")
                        .HasColumnName("tax");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.HasKey("Id")
                        .HasName("pk_invoices");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_invoices_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("Number")
                        .IsUnique()
                        .HasDatabaseName("ix_invoices_number");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_invoices_tenant_id_id");

                    b.HasIndex("TenantId", "IssuedAt")
                        .IsDescending(false, true)
                        .HasDatabaseName("ix_invoices_tenant_id_issued_at");

                    b.ToTable("invoices", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.MediaAsset", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<string>("FileName")
                        .IsRequired()
                        .HasMaxLength(260)
                        .HasColumnType("character varying(260)")
                        .HasColumnName("file_name");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<string>("Kind")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("kind");

                    b.Property<string>("MetaMediaId")
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("meta_media_id");

                    b.Property<string>("MimeType")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("mime_type");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<long>("SizeBytes")
                        .HasColumnType("bigint")
                        .HasColumnName("size_bytes");

                    b.Property<string>("StoragePath")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("storage_path");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.Property<DateTimeOffset>("UploadedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("uploaded_at");

                    b.HasKey("Id")
                        .HasName("pk_media_assets");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_media_assets_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("MetaMediaId")
                        .HasDatabaseName("ix_media_assets_meta_media_id")
                        .HasFilter("meta_media_id IS NOT NULL");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_media_assets_tenant_id_id");

                    b.HasIndex("TenantId", "UploadedAt")
                        .HasDatabaseName("ix_media_assets_tenant_id_uploaded_at");

                    b.ToTable("media_assets", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.MessageDailyStat", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<int>("Clicked")
                        .HasColumnType("integer")
                        .HasColumnName("clicked");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<DateOnly>("Date")
                        .HasColumnType("date")
                        .HasColumnName("date");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<int>("Delivered")
                        .HasColumnType("integer")
                        .HasColumnName("delivered");

                    b.Property<int>("Failed")
                        .HasColumnType("integer")
                        .HasColumnName("failed");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<int>("Read")
                        .HasColumnType("integer")
                        .HasColumnName("read");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<int>("Sent")
                        .HasColumnType("integer")
                        .HasColumnName("sent");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.HasKey("Id")
                        .HasName("pk_message_daily_stats");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_message_daily_stats_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("TenantId", "Date")
                        .IsUnique()
                        .HasDatabaseName("ix_message_daily_stats_tenant_id_date")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_message_daily_stats_tenant_id_id");

                    b.ToTable("message_daily_stats", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.MessageTemplate", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.PrimitiveCollection<List<string>>("BodyExamples")
                        .IsRequired()
                        .HasColumnType("text[]")
                        .HasColumnName("body_examples");

                    b.Property<string>("BodyText")
                        .IsRequired()
                        .HasMaxLength(2048)
                        .HasColumnType("character varying(2048)")
                        .HasColumnName("body_text");

                    b.PrimitiveCollection<List<string>>("Buttons")
                        .IsRequired()
                        .HasColumnType("text[]")
                        .HasColumnName("buttons");

                    b.Property<string>("Category")
                        .IsRequired()
                        .HasMaxLength(24)
                        .HasColumnType("character varying(24)")
                        .HasColumnName("category");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<string>("FooterText")
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("footer_text");

                    b.Property<string>("HeaderExample")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("header_example");

                    b.Property<string>("HeaderKind")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("header_kind");

                    b.Property<long?>("HeaderSampleId")
                        .HasColumnType("bigint")
                        .HasColumnName("header_sample_id");

                    b.Property<string>("HeaderText")
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("header_text");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<string>("Language")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("language");

                    b.Property<string>("MetaTemplateId")
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("meta_template_id");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("name");

                    b.Property<string>("QualityScore")
                        .IsRequired()
                        .HasMaxLength(8)
                        .HasColumnType("character varying(8)")
                        .HasColumnName("quality_score");

                    b.Property<string>("RejectionReason")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("rejection_reason");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("status");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.Property<int>("TimesUsed")
                        .HasColumnType("integer")
                        .HasColumnName("times_used");

                    b.PrimitiveCollection<List<string>>("Variables")
                        .IsRequired()
                        .HasColumnType("text[]")
                        .HasColumnName("variables");

                    b.Property<string>("WabaId")
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("waba_id");

                    b.HasKey("Id")
                        .HasName("pk_message_templates");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_message_templates_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_message_templates_tenant_id_id");

                    b.HasIndex("TenantId", "ModifiedOn")
                        .HasDatabaseName("ix_message_templates_tenant_id_modified_on");

                    b.HasIndex("TenantId", "Status")
                        .HasDatabaseName("ix_message_templates_tenant_id_status");

                    b.HasIndex("TenantId", "WabaId", "Name", "Language")
                        .IsUnique()
                        .HasDatabaseName("ix_message_templates_tenant_id_waba_id_name_language")
                        .HasFilter("is_deleted = false");

                    b.ToTable("message_templates", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.Notification", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<string>("ActionLabel")
                        .HasMaxLength(60)
                        .HasColumnType("character varying(60)")
                        .HasColumnName("action_label");

                    b.Property<string>("ActionRoute")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("action_route");

                    b.Property<string>("Body")
                        .IsRequired()
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)")
                        .HasColumnName("body");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<string>("Icon")
                        .IsRequired()
                        .HasMaxLength(40)
                        .HasColumnType("character varying(40)")
                        .HasColumnName("icon");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<string>("Kind")
                        .IsRequired()
                        .HasMaxLength(40)
                        .HasColumnType("character varying(40)")
                        .HasColumnName("kind");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<DateTimeOffset>("OccurredOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("occurred_on");

                    b.Property<string>("Priority")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("priority");

                    b.Property<bool>("Read")
                        .HasColumnType("boolean")
                        .HasColumnName("read");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("title");

                    b.Property<long?>("UserId")
                        .HasColumnType("bigint")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("pk_notifications");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_notifications_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_notifications_tenant_id_id");

                    b.HasIndex("TenantId", "UserId", "OccurredOn")
                        .IsDescending(false, false, true)
                        .HasDatabaseName("ix_notifications_tenant_id_user_id_occurred_on");

                    b.ToTable("notifications", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.NotificationDismissal", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<long>("NotificationId")
                        .HasColumnType("bigint")
                        .HasColumnName("notification_id");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.Property<long>("UserId")
                        .HasColumnType("bigint")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("pk_notification_dismissals");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_notification_dismissals_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("NotificationId")
                        .HasDatabaseName("ix_notification_dismissals_notification_id");

                    b.HasIndex("UserId", "NotificationId")
                        .IsUnique()
                        .HasDatabaseName("ix_notification_dismissals_user_id_notification_id")
                        .HasFilter("is_deleted = false");

                    b.ToTable("notification_dismissals", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.OutboxEmail", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<int>("AttemptCount")
                        .HasColumnType("integer")
                        .HasColumnName("attempt_count");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<string>("HtmlBody")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("html_body");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<string>("LastError")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("last_error");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<DateTimeOffset>("NextAttemptOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("next_attempt_on");

                    b.Property<string>("ReplyToAddress")
                        .HasMaxLength(320)
                        .HasColumnType("character varying(320)")
                        .HasColumnName("reply_to_address");

                    b.Property<string>("ReplyToName")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("reply_to_name");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<DateTimeOffset?>("SentOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("sent_on");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("status");

                    b.Property<string>("Subject")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("subject");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.Property<string>("TextBody")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("text_body");

                    b.Property<string>("ToAddress")
                        .IsRequired()
                        .HasMaxLength(320)
                        .HasColumnType("character varying(320)")
                        .HasColumnName("to_address");

                    b.Property<string>("ToName")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("to_name");

                    b.HasKey("Id")
                        .HasName("pk_outbox_emails");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_outbox_emails_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("NextAttemptOn")
                        .HasDatabaseName("ix_outbox_emails_next_attempt_on")
                        .HasFilter("is_deleted = false AND status = 'Pending'");

                    b.ToTable("outbox_emails", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.Payment", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<decimal>("Amount")
                        .HasPrecision(18, 4)
                        .HasColumnType("numeric(18,4)")
                        .HasColumnName("amount");

                    b.Property<string>("CardBrand")
                        .HasMaxLength(32)
                        .HasColumnType("character varying(32)")
                        .HasColumnName("card_brand");

                    b.Property<string>("CardLast4")
                        .HasMaxLength(4)
                        .HasColumnType("character varying(4)")
                        .HasColumnName("card_last4");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<string>("Currency")
                        .IsRequired()
                        .HasMaxLength(3)
                        .HasColumnType("character varying(3)")
                        .HasColumnName("currency");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<string>("FailureReason")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("failure_reason");

                    b.Property<long?>("InvoiceId")
                        .HasColumnType("bigint")
                        .HasColumnName("invoice_id");

                    b.Property<string>("InvoiceNumber")
                        .IsRequired()
                        .HasMaxLength(40)
                        .HasColumnType("character varying(40)")
                        .HasColumnName("invoice_number");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<string>("Method")
                        .IsRequired()
                        .HasMaxLength(24)
                        .HasColumnType("character varying(24)")
                        .HasColumnName("method");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<DateTimeOffset>("ProcessedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("processed_at");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("status");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.HasKey("Id")
                        .HasName("pk_payments");

                    b.HasIndex("InvoiceId")
                        .HasDatabaseName("ix_payments_invoice_id");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_payments_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_payments_tenant_id_id");

                    b.HasIndex("TenantId", "ProcessedAt")
                        .IsDescending(false, true)
                        .HasDatabaseName("ix_payments_tenant_id_processed_at");

                    b.ToTable("payments", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.PaymentChannelSetting", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<string>("AccountNumber")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("account_number");

                    b.Property<string>("AccountTitle")
                        .IsRequired()
                        .HasMaxLength(160)
                        .HasColumnType("character varying(160)")
                        .HasColumnName("account_title");

                    b.Property<string>("BankName")
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("bank_name");

                    b.Property<string>("Channel")
                        .IsRequired()
                        .HasMaxLength(24)
                        .HasColumnType("character varying(24)")
                        .HasColumnName("channel");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<string>("DisplayName")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("display_name");

                    b.PrimitiveCollection<List<string>>("Instructions")
                        .IsRequired()
                        .HasColumnType("text[]")
                        .HasColumnName("instructions");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean")
                        .HasColumnName("is_active");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<string>("QrContentType")
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("qr_content_type");

                    b.Property<string>("QrStorageKey")
                        .HasMaxLength(400)
                        .HasColumnType("character varying(400)")
                        .HasColumnName("qr_storage_key");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<int>("SortOrder")
                        .HasColumnType("integer")
                        .HasColumnName("sort_order");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.HasKey("Id")
                        .HasName("pk_payment_channel_settings");

                    b.HasIndex("Channel")
                        .IsUnique()
                        .HasDatabaseName("ix_payment_channel_settings_channel")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_payment_channel_settings_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.ToTable("payment_channel_settings", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.PaymentMethod", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<string>("Brand")
                        .HasMaxLength(40)
                        .HasColumnType("character varying(40)")
                        .HasColumnName("brand");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<int?>("ExpiryMonth")
                        .HasColumnType("integer")
                        .HasColumnName("expiry_month");

                    b.Property<int?>("ExpiryYear")
                        .HasColumnType("integer")
                        .HasColumnName("expiry_year");

                    b.Property<bool>("IsDefault")
                        .HasColumnType("boolean")
                        .HasColumnName("is_default");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<string>("Kind")
                        .IsRequired()
                        .HasMaxLength(24)
                        .HasColumnType("character varying(24)")
                        .HasColumnName("kind");

                    b.Property<string>("Last4")
                        .HasMaxLength(4)
                        .HasColumnType("character varying(4)")
                        .HasColumnName("last4");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<string>("ProviderToken")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)")
                        .HasColumnName("provider_token");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.HasKey("Id")
                        .HasName("pk_payment_methods");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_payment_methods_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("TenantId")
                        .IsUnique()
                        .HasDatabaseName("ix_payment_methods_tenant_id")
                        .HasFilter("is_default = true AND is_deleted = false");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_payment_methods_tenant_id_id");

                    b.HasIndex("TenantId", "ProviderToken")
                        .IsUnique()
                        .HasDatabaseName("ix_payment_methods_tenant_id_provider_token")
                        .HasFilter("is_deleted = false");

                    b.ToTable("payment_methods", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.PaymentRequest", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<decimal>("Amount")
                        .HasPrecision(18, 2)
                        .HasColumnType("numeric(18,2)")
                        .HasColumnName("amount");

                    b.Property<string>("BillingCycle")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("billing_cycle");

                    b.Property<string>("Channel")
                        .IsRequired()
                        .HasMaxLength(24)
                        .HasColumnType("character varying(24)")
                        .HasColumnName("channel");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<string>("Currency")
                        .IsRequired()
                        .HasMaxLength(3)
                        .HasColumnType("character varying(3)")
                        .HasColumnName("currency");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<long?>("InvoiceId")
                        .HasColumnType("bigint")
                        .HasColumnName("invoice_id");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<string>("Note")
                        .IsRequired()
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)")
                        .HasColumnName("note");

                    b.Property<string>("Organisation")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("organisation");

                    b.Property<string>("PlanName")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("plan_name");

                    b.Property<string>("ProofContentType")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("proof_content_type");

                    b.Property<string>("ProofFileName")
                        .IsRequired()
                        .HasMaxLength(260)
                        .HasColumnType("character varying(260)")
                        .HasColumnName("proof_file_name");

                    b.Property<long>("ProofSizeBytes")
                        .HasColumnType("bigint")
                        .HasColumnName("proof_size_bytes");

                    b.Property<string>("ProofStorageKey")
                        .IsRequired()
                        .HasMaxLength(400)
                        .HasColumnType("character varying(400)")
                        .HasColumnName("proof_storage_key");

                    b.Property<string>("Reference")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("reference");

                    b.Property<string>("RejectionReason")
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)")
                        .HasColumnName("rejection_reason");

                    b.Property<DateTimeOffset?>("ReviewedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("reviewed_at");

                    b.Property<string>("ReviewedByName")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("reviewed_by_name");

                    b.Property<long?>("ReviewedByUserId")
                        .HasColumnType("bigint")
                        .HasColumnName("reviewed_by_user_id");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("status");

                    b.Property<DateTimeOffset>("SubmittedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("submitted_at");

                    b.Property<string>("SubmittedByEmail")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)")
                        .HasColumnName("submitted_by_email");

                    b.Property<string>("SubmittedByName")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("submitted_by_name");

                    b.Property<long>("SubmittedByUserId")
                        .HasColumnType("bigint")
                        .HasColumnName("submitted_by_user_id");

                    b.Property<long>("SubscriptionPlanId")
                        .HasColumnType("bigint")
                        .HasColumnName("subscription_plan_id");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.HasKey("Id")
                        .HasName("pk_payment_requests");

                    b.HasIndex("InvoiceId")
                        .HasDatabaseName("ix_payment_requests_invoice_id");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_payment_requests_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("SubscriptionPlanId")
                        .HasDatabaseName("ix_payment_requests_subscription_plan_id");

                    b.HasIndex("Status", "SubmittedAt")
                        .HasDatabaseName("ix_payment_requests_status_submitted_at");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_payment_requests_tenant_id_id");

                    b.HasIndex("TenantId", "Status", "SubmittedAt")
                        .HasDatabaseName("ix_payment_requests_tenant_id_status_submitted_at");

                    b.ToTable("payment_requests", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.PermissionSet", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("description");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<bool>("IsSystem")
                        .HasColumnType("boolean")
                        .HasColumnName("is_system");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("name");

                    b.PrimitiveCollection<List<string>>("Permissions")
                        .IsRequired()
                        .HasColumnType("text[]")
                        .HasColumnName("permissions");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.HasKey("Id")
                        .HasName("pk_permission_sets");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_permission_sets_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_permission_sets_tenant_id_id");

                    b.HasIndex("TenantId", "Name")
                        .IsUnique()
                        .HasDatabaseName("ix_permission_sets_tenant_id_name")
                        .HasFilter("is_deleted = false");

                    b.ToTable("permission_sets", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.RefreshToken", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<DateTimeOffset?>("ConsumedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("consumed_on");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<string>("CreatedByIp")
                        .HasMaxLength(45)
                        .HasColumnType("character varying(45)")
                        .HasColumnName("created_by_ip");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<DateTimeOffset>("ExpiresOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("expires_on");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<string>("ReplacedByTokenHash")
                        .HasMaxLength(64)
                        .HasColumnType("character(64)")
                        .HasColumnName("replaced_by_token_hash")
                        .IsFixedLength();

                    b.Property<DateTimeOffset?>("RevokedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("revoked_on");

                    b.Property<string>("RevokedReason")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)")
                        .HasColumnName("revoked_reason");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<Guid>("SecurityStamp")
                        .HasColumnType("uuid")
                        .HasColumnName("security_stamp");

                    b.Property<Guid>("SessionId")
                        .HasColumnType("uuid")
                        .HasColumnName("session_id");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.Property<string>("TokenHash")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("character(64)")
                        .HasColumnName("token_hash")
                        .IsFixedLength();

                    b.Property<string>("UserAgent")
                        .HasMaxLength(512)
                        .HasColumnType("character varying(512)")
                        .HasColumnName("user_agent");

                    b.Property<long>("UserId")
                        .HasColumnType("bigint")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("pk_refresh_tokens");

                    b.HasIndex("ExpiresOn")
                        .HasDatabaseName("ix_refresh_tokens_expires_on");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_refresh_tokens_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("SessionId")
                        .HasDatabaseName("ix_refresh_tokens_session_id");

                    b.HasIndex("TokenHash")
                        .IsUnique()
                        .HasDatabaseName("ix_refresh_tokens_token_hash");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_refresh_tokens_tenant_id_id");

                    b.HasIndex("UserId", "SessionId")
                        .HasDatabaseName("ix_refresh_tokens_user_id_session_id");

                    b.ToTable("refresh_tokens", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.RenewalRecord", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<decimal>("Amount")
                        .HasPrecision(18, 4)
                        .HasColumnType("numeric(18,4)")
                        .HasColumnName("amount");

                    b.Property<bool>("Automatic")
                        .HasColumnType("boolean")
                        .HasColumnName("automatic");

                    b.Property<string>("BillingCycle")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("billing_cycle");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<string>("Currency")
                        .IsRequired()
                        .HasMaxLength(3)
                        .HasColumnType("character varying(3)")
                        .HasColumnName("currency");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<DateTimeOffset>("PeriodEnd")
                        .HasColumnType("timestamptz")
                        .HasColumnName("period_end");

                    b.Property<string>("PlanName")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("plan_name");

                    b.Property<DateTimeOffset>("RenewedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("renewed_at");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.HasKey("Id")
                        .HasName("pk_renewal_records");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_renewal_records_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_renewal_records_tenant_id_id");

                    b.HasIndex("TenantId", "RenewedAt")
                        .IsDescending(false, true)
                        .HasDatabaseName("ix_renewal_records_tenant_id_renewed_at");

                    b.ToTable("renewal_records", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.Role", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)")
                        .HasColumnName("description");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<bool>("IsSystemRole")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_system_role");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("name");

                    b.Property<string>("NormalizedName")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("normalized_name");

                    b.PrimitiveCollection<List<string>>("Permissions")
                        .IsRequired()
                        .HasColumnType("text[]")
                        .HasColumnName("permissions");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.HasKey("Id")
                        .HasName("pk_roles");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_roles_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("NormalizedName")
                        .IsUnique()
                        .HasDatabaseName("ix_roles_normalized_name")
                        .HasFilter("is_deleted = false");

                    b.ToTable("roles", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.SecurityEvent", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<string>("Detail")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("detail");

                    b.Property<string>("DeviceLabel")
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("device_label");

                    b.Property<string>("IpAddress")
                        .HasMaxLength(45)
                        .HasColumnType("character varying(45)")
                        .HasColumnName("ip_address");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<string>("Kind")
                        .IsRequired()
                        .HasMaxLength(32)
                        .HasColumnType("character varying(32)")
                        .HasColumnName("kind");

                    b.Property<string>("Location")
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("location");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<DateTimeOffset>("OccurredAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("occurred_at");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<Guid?>("SessionId")
                        .HasColumnType("uuid")
                        .HasColumnName("session_id");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.Property<long>("UserId")
                        .HasColumnType("bigint")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("pk_security_events");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_security_events_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_security_events_tenant_id_id");

                    b.HasIndex("UserId", "Kind", "OccurredAt")
                        .HasDatabaseName("ix_security_events_user_id_kind_occurred_at");

                    b.ToTable("security_events", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.SubscriptionPlan", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.PrimitiveCollection<List<string>>("AutoReplyTriggers")
                        .IsRequired()
                        .HasColumnType("text[]")
                        .HasColumnName("auto_reply_triggers");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<string>("Currency")
                        .IsRequired()
                        .HasMaxLength(3)
                        .HasColumnType("character varying(3)")
                        .HasColumnName("currency");

                    b.Property<int?>("DailyMessageLimit")
                        .HasColumnType("integer")
                        .HasColumnName("daily_message_limit");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<decimal>("DiscountPercent")
                        .HasPrecision(18, 4)
                        .HasColumnType("numeric(18,4)")
                        .HasColumnName("discount_percent");

                    b.PrimitiveCollection<List<string>>("EnabledModules")
                        .IsRequired()
                        .HasColumnType("text[]")
                        .HasColumnName("enabled_modules");

                    b.PrimitiveCollection<List<string>>("Highlights")
                        .IsRequired()
                        .HasColumnType("text[]")
                        .HasColumnName("highlights");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<bool>("IsMostPopular")
                        .HasColumnType("boolean")
                        .HasColumnName("is_most_popular");

                    b.Property<bool>("IsPromotional")
                        .HasColumnType("boolean")
                        .HasColumnName("is_promotional");

                    b.Property<bool>("IsRecommended")
                        .HasColumnType("boolean")
                        .HasColumnName("is_recommended");

                    b.Property<int?>("MaxApiCallsPerMonth")
                        .HasColumnType("integer")
                        .HasColumnName("max_api_calls_per_month");

                    b.Property<int?>("MaxCampaigns")
                        .HasColumnType("integer")
                        .HasColumnName("max_campaigns");

                    b.Property<int?>("MaxContacts")
                        .HasColumnType("integer")
                        .HasColumnName("max_contacts");

                    b.Property<int?>("MaxEmailAccounts")
                        .HasColumnType("integer")
                        .HasColumnName("max_email_accounts");

                    b.Property<int?>("MaxEmployees")
                        .HasColumnType("integer")
                        .HasColumnName("max_employees");

                    b.Property<int?>("MaxSearchRadiusKm")
                        .HasColumnType("integer")
                        .HasColumnName("max_search_radius_km");

                    b.Property<int?>("MaxSocialAccounts")
                        .HasColumnType("integer")
                        .HasColumnName("max_social_accounts");

                    b.Property<int?>("MaxStorageMb")
                        .HasColumnType("integer")
                        .HasColumnName("max_storage_mb");

                    b.Property<int?>("MaxWhatsAppAccounts")
                        .HasColumnType("integer")
                        .HasColumnName("max_whats_app_accounts");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<int?>("MonthlyAiReplyLimit")
                        .HasColumnType("integer")
                        .HasColumnName("monthly_ai_reply_limit");

                    b.Property<int?>("MonthlyMessageLimit")
                        .HasColumnType("integer")
                        .HasColumnName("monthly_message_limit");

                    b.Property<decimal>("MonthlyPrice")
                        .HasPrecision(18, 4)
                        .HasColumnType("numeric(18,4)")
                        .HasColumnName("monthly_price");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("name");

                    b.Property<int>("RenewalPeriodMonths")
                        .HasColumnType("integer")
                        .HasColumnName("renewal_period_months");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<int>("SortOrder")
                        .HasColumnType("integer")
                        .HasColumnName("sort_order");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("status");

                    b.Property<string>("SupportLevel")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("support_level");

                    b.Property<string>("Tagline")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("tagline");

                    b.Property<int>("TrialDays")
                        .HasColumnType("integer")
                        .HasColumnName("trial_days");

                    b.Property<decimal>("YearlyPrice")
                        .HasPrecision(18, 4)
                        .HasColumnType("numeric(18,4)")
                        .HasColumnName("yearly_price");

                    b.HasKey("Id")
                        .HasName("pk_subscription_plans");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_subscription_plans_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("Status", "SortOrder")
                        .HasDatabaseName("ix_subscription_plans_status_sort_order");

                    b.ToTable("subscription_plans", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.TemplateHeaderSample", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<string>("FileName")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("file_name");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<string>("Kind")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("kind");

                    b.Property<long?>("MessageTemplateId")
                        .HasColumnType("bigint")
                        .HasColumnName("message_template_id");

                    b.Property<string>("MimeType")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("mime_type");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<long>("SizeBytes")
                        .HasColumnType("bigint")
                        .HasColumnName("size_bytes");

                    b.Property<string>("StoragePath")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("storage_path");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.Property<DateTimeOffset>("UploadedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("uploaded_at");

                    b.HasKey("Id")
                        .HasName("pk_template_header_samples");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_template_header_samples_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("MessageTemplateId", "UploadedAt")
                        .HasDatabaseName("ix_template_header_samples_message_template_id_uploaded_at");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_template_header_samples_tenant_id_id");

                    b.ToTable("template_header_samples", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.Tenant", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<DateTimeOffset?>("ActivatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("activated_on");

                    b.Property<string>("ContactEmail")
                        .IsRequired()
                        .HasMaxLength(320)
                        .HasColumnType("character varying(320)")
                        .HasColumnName("contact_email");

                    b.Property<int>("ContactQuota")
                        .HasColumnType("integer")
                        .HasColumnName("contact_quota");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<string>("CurrencyCode")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(3)
                        .HasColumnType("character varying(3)")
                        .HasDefaultValue("USD")
                        .HasColumnName("currency_code");

                    b.Property<DateOnly?>("DataRetainedUntil")
                        .HasColumnType("date")
                        .HasColumnName("data_retained_until");

                    b.Property<long?>("DeactivatedByUserId")
                        .HasColumnType("bigint")
                        .HasColumnName("deactivated_by_user_id");

                    b.Property<DateTimeOffset?>("DeactivatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deactivated_on");

                    b.Property<string>("DeactivationDetails")
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)")
                        .HasColumnName("deactivation_details");

                    b.Property<string>("DeactivationReason")
                        .HasMaxLength(24)
                        .HasColumnType("character varying(24)")
                        .HasColumnName("deactivation_reason");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<DateTimeOffset?>("LastActiveOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("last_active_on");

                    b.Property<int>("MessagesThisMonth")
                        .HasColumnType("integer")
                        .HasColumnName("messages_this_month");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<int>("MonthlyMessageQuota")
                        .HasColumnType("integer")
                        .HasColumnName("monthly_message_quota");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("name");

                    b.Property<int>("PlanBand")
                        .HasColumnType("integer")
                        .HasColumnName("plan_band");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<string>("Slug")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("slug");

                    b.Property<string>("Status")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(32)
                        .HasColumnType("character varying(32)")
                        .HasDefaultValue("Pending")
                        .HasColumnName("status");

                    b.Property<DateTimeOffset?>("SuspendedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("suspended_on");

                    b.Property<string>("TimeZoneId")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasDefaultValue("UTC")
                        .HasColumnName("time_zone_id");

                    b.HasKey("Id")
                        .HasName("pk_tenants");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_tenants_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("Slug")
                        .IsUnique()
                        .HasDatabaseName("ix_tenants_slug")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("Status")
                        .HasDatabaseName("ix_tenants_status");

                    b.ToTable("tenants", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.TenantSubscription", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<decimal>("Amount")
                        .HasPrecision(18, 4)
                        .HasColumnType("numeric(18,4)")
                        .HasColumnName("amount");

                    b.Property<bool>("AutoRenew")
                        .HasColumnType("boolean")
                        .HasColumnName("auto_renew");

                    b.Property<string>("BillingCycle")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("billing_cycle");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<string>("Currency")
                        .IsRequired()
                        .HasMaxLength(3)
                        .HasColumnType("character varying(3)")
                        .HasColumnName("currency");

                    b.Property<DateTimeOffset>("CurrentPeriodEnd")
                        .HasColumnType("timestamptz")
                        .HasColumnName("current_period_end");

                    b.Property<DateTimeOffset>("CurrentPeriodStart")
                        .HasColumnType("timestamptz")
                        .HasColumnName("current_period_start");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<DateTimeOffset>("ExpiresAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("expires_at");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<int?>("LastExpiryReminderDay")
                        .HasColumnType("integer")
                        .HasColumnName("last_expiry_reminder_day");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<DateTimeOffset?>("NextRenewalAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("next_renewal_at");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<int>("SeatsPurchased")
                        .HasColumnType("integer")
                        .HasColumnName("seats_purchased");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("status");

                    b.Property<long>("SubscriptionPlanId")
                        .HasColumnType("bigint")
                        .HasColumnName("subscription_plan_id");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.Property<DateTimeOffset?>("TrialEndsAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("trial_ends_at");

                    b.HasKey("Id")
                        .HasName("pk_tenant_subscriptions");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_tenant_subscriptions_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("SubscriptionPlanId")
                        .HasDatabaseName("ix_tenant_subscriptions_subscription_plan_id");

                    b.HasIndex("TenantId")
                        .IsUnique()
                        .HasDatabaseName("ix_tenant_subscriptions_tenant_id")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_tenant_subscriptions_tenant_id_id");

                    b.ToTable("tenant_subscriptions", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.User", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<string>("AvatarUrl")
                        .HasColumnType("text")
                        .HasColumnName("avatar_url");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DefaultWhatsAppConnectionId")
                        .HasColumnType("bigint")
                        .HasColumnName("default_whats_app_connection_id");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<string>("DisplayName")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)")
                        .HasColumnName("display_name");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(320)
                        .HasColumnType("character varying(320)")
                        .HasColumnName("email");

                    b.Property<bool>("EmailConfirmed")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("email_confirmed");

                    b.Property<int>("FailedLoginAttempts")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0)
                        .HasColumnName("failed_login_attempts");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<string>("JobTitle")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("job_title");

                    b.Property<DateTimeOffset?>("LastLoginOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("last_login_on");

                    b.Property<DateTimeOffset?>("LockoutEndsOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("lockout_ends_on");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<string>("NormalizedEmail")
                        .IsRequired()
                        .HasMaxLength(320)
                        .HasColumnType("character varying(320)")
                        .HasColumnName("normalized_email");

                    b.Property<string>("OnboardingStatus")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("onboarding_status");

                    b.Property<int>("OnboardingStepIndex")
                        .HasColumnType("integer")
                        .HasColumnName("onboarding_step_index");

                    b.Property<DateTimeOffset?>("OnboardingUpdatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("onboarding_updated_on");

                    b.Property<string>("PasswordHash")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)")
                        .HasColumnName("password_hash");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<Guid>("SecurityStamp")
                        .HasColumnType("uuid")
                        .HasColumnName("security_stamp");

                    b.Property<string>("Status")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(32)
                        .HasColumnType("character varying(32)")
                        .HasDefaultValue("Invited")
                        .HasColumnName("status");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.HasKey("Id")
                        .HasName("pk_users");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_users_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("NormalizedEmail")
                        .IsUnique()
                        .HasDatabaseName("ix_users_normalized_email")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_users_tenant_id_id");

                    b.HasIndex("TenantId", "Status")
                        .HasDatabaseName("ix_users_tenant_id_status");

                    b.ToTable("users", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.UserNotificationPreference", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<string>("Category")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("category");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<bool>("Enabled")
                        .HasColumnType("boolean")
                        .HasColumnName("enabled");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.Property<long>("UserId")
                        .HasColumnType("bigint")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("pk_user_notification_preferences");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_user_notification_preferences_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("UserId", "Category")
                        .IsUnique()
                        .HasDatabaseName("ix_user_notification_preferences_user_id_category")
                        .HasFilter("is_deleted = false");

                    b.ToTable("user_notification_preferences", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.UserPermissionOverride", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<bool>("IsGranted")
                        .HasColumnType("boolean")
                        .HasColumnName("is_granted");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<string>("Permission")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("permission");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.Property<long>("UserId")
                        .HasColumnType("bigint")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("pk_user_permission_overrides");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_user_permission_overrides_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_user_permission_overrides_tenant_id_id");

                    b.HasIndex("UserId", "Permission")
                        .IsUnique()
                        .HasDatabaseName("ix_user_permission_overrides_user_id_permission")
                        .HasFilter("is_deleted = false");

                    b.ToTable("user_permission_overrides", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.UserRole", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<long>("RoleId")
                        .HasColumnType("bigint")
                        .HasColumnName("role_id");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.Property<long>("UserId")
                        .HasColumnType("bigint")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("pk_user_roles");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_user_roles_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("RoleId")
                        .HasDatabaseName("ix_user_roles_role_id");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_user_roles_tenant_id_id");

                    b.HasIndex("UserId", "RoleId")
                        .IsUnique()
                        .HasDatabaseName("ix_user_roles_user_id_role_id")
                        .HasFilter("is_deleted = false");

                    b.ToTable("user_roles", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.UserSession", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<string>("Browser")
                        .IsRequired()
                        .HasMaxLength(40)
                        .HasColumnType("character varying(40)")
                        .HasColumnName("browser");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<string>("DeviceId")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("device_id");

                    b.Property<string>("DeviceLabel")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("device_label");

                    b.Property<string>("DeviceType")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("device_type");

                    b.Property<string>("IpAddress")
                        .HasMaxLength(45)
                        .HasColumnType("character varying(45)")
                        .HasColumnName("ip_address");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<DateTimeOffset>("LastActivityAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("last_activity_at");

                    b.Property<string>("LastIpAddress")
                        .HasMaxLength(45)
                        .HasColumnType("character varying(45)")
                        .HasColumnName("last_ip_address");

                    b.Property<string>("Location")
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("location");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<string>("OperatingSystem")
                        .IsRequired()
                        .HasMaxLength(40)
                        .HasColumnType("character varying(40)")
                        .HasColumnName("operating_system");

                    b.Property<DateTimeOffset?>("RevokedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("revoked_at");

                    b.Property<long?>("RevokedByUserId")
                        .HasColumnType("bigint")
                        .HasColumnName("revoked_by_user_id");

                    b.Property<string>("RevokedReason")
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("revoked_reason");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<Guid>("SessionId")
                        .HasColumnType("uuid")
                        .HasColumnName("session_id");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.Property<string>("UserAgent")
                        .HasMaxLength(512)
                        .HasColumnType("character varying(512)")
                        .HasColumnName("user_agent");

                    b.Property<long>("UserId")
                        .HasColumnType("bigint")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("pk_user_sessions");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_user_sessions_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("SessionId")
                        .IsUnique()
                        .HasDatabaseName("ix_user_sessions_session_id");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_user_sessions_tenant_id_id");

                    b.HasIndex("TenantId", "LastActivityAt")
                        .HasDatabaseName("ix_user_sessions_tenant_id_last_activity_at");

                    b.HasIndex("UserId", "LastActivityAt")
                        .HasDatabaseName("ix_user_sessions_user_id_last_activity_at");

                    b.ToTable("user_sessions", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.UserToken", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<DateTimeOffset?>("ConsumedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("consumed_on");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<DateTimeOffset>("ExpiresOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("expires_on");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<string>("Purpose")
                        .IsRequired()
                        .HasMaxLength(24)
                        .HasColumnType("character varying(24)")
                        .HasColumnName("purpose");

                    b.Property<string>("RequestedByIp")
                        .HasMaxLength(45)
                        .HasColumnType("character varying(45)")
                        .HasColumnName("requested_by_ip");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.Property<string>("TokenHash")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("character(64)")
                        .HasColumnName("token_hash")
                        .IsFixedLength();

                    b.Property<long>("UserId")
                        .HasColumnType("bigint")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("pk_user_tokens");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_user_tokens_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("TokenHash")
                        .IsUnique()
                        .HasDatabaseName("ix_user_tokens_token_hash");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_user_tokens_tenant_id_id");

                    b.HasIndex("UserId", "Purpose", "ConsumedOn")
                        .HasDatabaseName("ix_user_tokens_user_id_purpose_consumed_on");

                    b.ToTable("user_tokens", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.WhatsAppAccountAccess", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<bool>("CanBroadcast")
                        .HasColumnType("boolean")
                        .HasColumnName("can_broadcast");

                    b.Property<bool>("CanReply")
                        .HasColumnType("boolean")
                        .HasColumnName("can_reply");

                    b.Property<bool>("CanView")
                        .HasColumnType("boolean")
                        .HasColumnName("can_view");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.Property<long>("UserId")
                        .HasColumnType("bigint")
                        .HasColumnName("user_id");

                    b.Property<long>("WhatsAppConnectionId")
                        .HasColumnType("bigint")
                        .HasColumnName("whats_app_connection_id");

                    b.HasKey("Id")
                        .HasName("pk_whatsapp_account_access");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_whatsapp_account_access_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("WhatsAppConnectionId")
                        .HasDatabaseName("ix_whatsapp_account_access_whats_app_connection_id");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_whatsapp_account_access_tenant_id_id");

                    b.HasIndex("UserId", "WhatsAppConnectionId")
                        .IsUnique()
                        .HasDatabaseName("ix_whatsapp_account_access_user_id_whats_app_connection_id")
                        .HasFilter("is_deleted = false");

                    b.ToTable("whatsapp_account_access", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.WhatsAppConnection", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<string>("AccountStatus")
                        .HasMaxLength(32)
                        .HasColumnType("character varying(32)")
                        .HasColumnName("account_status");

                    b.Property<string>("ApiStatus")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("api_status");

                    b.Property<string>("BusinessCategory")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("business_category");

                    b.Property<string>("BusinessProfileAbout")
                        .IsRequired()
                        .HasMaxLength(512)
                        .HasColumnType("character varying(512)")
                        .HasColumnName("business_profile_about");

                    b.Property<DateTimeOffset?>("ConnectedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("connected_at");

                    b.Property<long>("CreatedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("created_by");

                    b.Property<DateTimeOffset>("CreatedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_on");

                    b.Property<long?>("DeletedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("deleted_by");

                    b.Property<DateTimeOffset?>("DeletedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("deleted_on");

                    b.Property<string>("DisplayPhoneNumber")
                        .IsRequired()
                        .HasMaxLength(32)
                        .HasColumnType("character varying(32)")
                        .HasColumnName("display_phone_number");

                    b.Property<string>("EncryptedAccessToken")
                        .HasMaxLength(2048)
                        .HasColumnType("character varying(2048)")
                        .HasColumnName("encrypted_access_token");

                    b.Property<bool>("IsDefault")
                        .HasColumnType("boolean")
                        .HasColumnName("is_default");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_deleted");

                    b.Property<string>("Label")
                        .IsRequired()
                        .HasMaxLength(40)
                        .HasColumnType("character varying(40)")
                        .HasColumnName("label");

                    b.Property<string>("LastError")
                        .HasMaxLength(300)
                        .HasColumnType("character varying(300)")
                        .HasColumnName("last_error");

                    b.Property<DateTimeOffset?>("LastMessageReceivedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("last_message_received_at");

                    b.Property<DateTimeOffset?>("LastMessageSentAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("last_message_sent_at");

                    b.Property<DateTimeOffset?>("LastWebhookAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("last_webhook_at");

                    b.Property<int>("MessagesLast24h")
                        .HasColumnType("integer")
                        .HasColumnName("messages_last24h");

                    b.Property<int>("MessagingLimit")
                        .HasColumnType("integer")
                        .HasColumnName("messaging_limit");

                    b.Property<string>("MessagingTier")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("messaging_tier");

                    b.Property<long?>("ModifiedBy")
                        .HasColumnType("bigint")
                        .HasColumnName("modified_by");

                    b.Property<DateTimeOffset?>("ModifiedOn")
                        .HasColumnType("timestamptz")
                        .HasColumnName("modified_on");

                    b.Property<string>("PhoneNumberId")
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("phone_number_id");

                    b.Property<string>("PhoneNumberStatus")
                        .HasMaxLength(32)
                        .HasColumnType("character varying(32)")
                        .HasColumnName("phone_number_status");

                    b.Property<string>("QualityRating")
                        .IsRequired()
                        .HasMaxLength(8)
                        .HasColumnType("character varying(8)")
                        .HasColumnName("quality_rating");

                    b.Property<string>("RegistrationPin")
                        .HasMaxLength(6)
                        .HasColumnType("character(6)")
                        .HasColumnName("registration_pin")
                        .IsFixedLength();

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("status");

                    b.Property<string>("TemplateNamespaceAlias")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("template_namespace_alias");

                    b.Property<long?>("TenantId")
                        .HasColumnType("bigint")
                        .HasColumnName("tenant_id");

                    b.Property<DateTimeOffset?>("TokenExpiresAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("token_expires_at");

                    b.Property<string>("VerifiedName")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("verified_name");

                    b.Property<string>("WabaId")
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("waba_id");

                    b.Property<bool>("WebhookHealthy")
                        .HasColumnType("boolean")
                        .HasColumnName("webhook_healthy");

                    b.HasKey("Id")
                        .HasName("pk_whatsapp_connections");

                    b.HasIndex("IsDeleted")
                        .HasDatabaseName("ix_whatsapp_connections_is_deleted")
                        .HasFilter("is_deleted = false");

                    b.HasIndex("PhoneNumberId")
                        .IsUnique()
                        .HasDatabaseName("ix_whatsapp_connections_phone_number_id")
                        .HasFilter("is_deleted = false AND status <> 'Disconnected'");

                    b.HasIndex("TenantId")
                        .IsUnique()
                        .HasDatabaseName("ix_whatsapp_connections_tenant_id_default")
                        .HasFilter("is_default = true AND is_deleted = false");

                    b.HasIndex("TenantId", "Id")
                        .HasDatabaseName("ix_whatsapp_connections_tenant_id_id");

                    b.ToTable("whatsapp_connections", (string)null);
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.Campaign", b =>
                {
                    b.HasOne("Marketing.DataAccess.Entities.MessageTemplate", "MessageTemplate")
                        .WithMany()
                        .HasForeignKey("MessageTemplateId")
                        .OnDelete(DeleteBehavior.SetNull)
                        .HasConstraintName("fk_campaigns_message_templates_message_template_id");

                    b.Navigation("MessageTemplate");
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.CampaignMessage", b =>
                {
                    b.HasOne("Marketing.DataAccess.Entities.Campaign", "Campaign")
                        .WithMany()
                        .HasForeignKey("CampaignId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_campaign_messages_campaigns_campaign_id");

                    b.HasOne("Marketing.DataAccess.Entities.CampaignRun", "CampaignRun")
                        .WithMany("Messages")
                        .HasForeignKey("CampaignRunId")
                        .OnDelete(DeleteBehavior.SetNull)
                        .HasConstraintName("fk_campaign_messages_campaign_runs_campaign_run_id");

                    b.HasOne("Marketing.DataAccess.Entities.Contact", "Contact")
                        .WithMany()
                        .HasForeignKey("ContactId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_campaign_messages_contacts_contact_id");

                    b.Navigation("Campaign");

                    b.Navigation("CampaignRun");

                    b.Navigation("Contact");
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.CampaignRun", b =>
                {
                    b.HasOne("Marketing.DataAccess.Entities.Campaign", "Campaign")
                        .WithMany("Runs")
                        .HasForeignKey("CampaignId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_campaign_runs_campaigns_campaign_id");

                    b.Navigation("Campaign");
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.ContactGroupMember", b =>
                {
                    b.HasOne("Marketing.DataAccess.Entities.ContactGroup", "ContactGroup")
                        .WithMany("Members")
                        .HasForeignKey("ContactGroupId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_contact_group_members_contact_groups_contact_group_id");

                    b.HasOne("Marketing.DataAccess.Entities.Contact", "Contact")
                        .WithMany("GroupMemberships")
                        .HasForeignKey("ContactId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_contact_group_members_contacts_contact_id");

                    b.Navigation("Contact");

                    b.Navigation("ContactGroup");
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.ContactImportExport", b =>
                {
                    b.HasOne("Marketing.DataAccess.Entities.ContactImportBatch", "ContactImportBatch")
                        .WithMany("Exports")
                        .HasForeignKey("ContactImportBatchId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_contact_import_exports_contact_import_batches_contact_impor");

                    b.Navigation("ContactImportBatch");
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.ContactImportRow", b =>
                {
                    b.HasOne("Marketing.DataAccess.Entities.ContactImportBatch", "ContactImportBatch")
                        .WithMany("Rows")
                        .HasForeignKey("ContactImportBatchId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_contact_import_rows_contact_import_batches_contact_import_b");

                    b.Navigation("ContactImportBatch");
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.ContactTagAssignment", b =>
                {
                    b.HasOne("Marketing.DataAccess.Entities.Contact", "Contact")
                        .WithMany("TagAssignments")
                        .HasForeignKey("ContactId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_contact_tag_assignments_contacts_contact_id");

                    b.HasOne("Marketing.DataAccess.Entities.ContactTag", "ContactTag")
                        .WithMany("Assignments")
                        .HasForeignKey("ContactTagId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_contact_tag_assignments_contact_tags_contact_tag_id");

                    b.Navigation("Contact");

                    b.Navigation("ContactTag");
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.ConversationMessage", b =>
                {
                    b.HasOne("Marketing.DataAccess.Entities.Conversation", "Conversation")
                        .WithMany("Messages")
                        .HasForeignKey("ConversationId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_conversation_messages_conversations_conversation_id");

                    b.HasOne("Marketing.DataAccess.Entities.MediaAsset", "Media")
                        .WithMany()
                        .HasForeignKey("MediaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_conversation_messages_media_asset_media_id");

                    b.Navigation("Conversation");

                    b.Navigation("Media");
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.DeliveryFailure", b =>
                {
                    b.HasOne("Marketing.DataAccess.Entities.Campaign", "Campaign")
                        .WithMany()
                        .HasForeignKey("CampaignId")
                        .OnDelete(DeleteBehavior.SetNull)
                        .HasConstraintName("fk_delivery_failures_campaigns_campaign_id");

                    b.Navigation("Campaign");
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.ImportJob", b =>
                {
                    b.HasOne("Marketing.DataAccess.Entities.ContactImportBatch", "ContactImportBatch")
                        .WithMany()
                        .HasForeignKey("ContactImportBatchId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_import_jobs_contact_import_batches_contact_import_batch_id");

                    b.HasOne("Marketing.DataAccess.Entities.ContactImportExport", "ContactImportExport")
                        .WithMany()
                        .HasForeignKey("ContactImportExportId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .HasConstraintName("fk_import_jobs_contact_import_exports_contact_import_export_id");

                    b.Navigation("ContactImportBatch");

                    b.Navigation("ContactImportExport");
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.NotificationDismissal", b =>
                {
                    b.HasOne("Marketing.DataAccess.Entities.Notification", null)
                        .WithMany()
                        .HasForeignKey("NotificationId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_notification_dismissals_notifications_notification_id");
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.Payment", b =>
                {
                    b.HasOne("Marketing.DataAccess.Entities.Invoice", "Invoice")
                        .WithMany()
                        .HasForeignKey("InvoiceId")
                        .OnDelete(DeleteBehavior.SetNull)
                        .HasConstraintName("fk_payments_invoices_invoice_id");

                    b.Navigation("Invoice");
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.PaymentRequest", b =>
                {
                    b.HasOne("Marketing.DataAccess.Entities.Invoice", "Invoice")
                        .WithMany()
                        .HasForeignKey("InvoiceId")
                        .HasConstraintName("fk_payment_requests_invoices_invoice_id");

                    b.HasOne("Marketing.DataAccess.Entities.SubscriptionPlan", "SubscriptionPlan")
                        .WithMany()
                        .HasForeignKey("SubscriptionPlanId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_payment_requests_subscription_plans_subscription_plan_id");

                    b.Navigation("Invoice");

                    b.Navigation("SubscriptionPlan");
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.RefreshToken", b =>
                {
                    b.HasOne("Marketing.DataAccess.Entities.User", "User")
                        .WithMany("RefreshTokens")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_refresh_tokens_users_user_id");

                    b.Navigation("User");
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.TenantSubscription", b =>
                {
                    b.HasOne("Marketing.DataAccess.Entities.SubscriptionPlan", "SubscriptionPlan")
                        .WithMany()
                        .HasForeignKey("SubscriptionPlanId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_tenant_subscriptions_subscription_plans_subscription_plan_id");

                    b.Navigation("SubscriptionPlan");
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.User", b =>
                {
                    b.HasOne("Marketing.DataAccess.Entities.Tenant", "Tenant")
                        .WithMany("Users")
                        .HasForeignKey("TenantId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_users_tenants_tenant_id");

                    b.Navigation("Tenant");
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.UserPermissionOverride", b =>
                {
                    b.HasOne("Marketing.DataAccess.Entities.User", "User")
                        .WithMany("PermissionOverrides")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_user_permission_overrides_users_user_id");

                    b.Navigation("User");
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.UserRole", b =>
                {
                    b.HasOne("Marketing.DataAccess.Entities.Role", "Role")
                        .WithMany("UserRoles")
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_user_roles_roles_role_id");

                    b.HasOne("Marketing.DataAccess.Entities.User", "User")
                        .WithMany("UserRoles")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_user_roles_users_user_id");

                    b.Navigation("Role");

                    b.Navigation("User");
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.UserToken", b =>
                {
                    b.HasOne("Marketing.DataAccess.Entities.User", "User")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_user_tokens_users_user_id");

                    b.Navigation("User");
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.WhatsAppConnection", b =>
                {
                    b.OwnsMany("Marketing.DataAccess.Entities.WhatsAppOnboardingStep", "OnboardingSteps", b1 =>
                        {
                            b1.Property<long>("WhatsAppConnectionId");

                            b1.Property<int>("__synthesizedOrdinal")
                                .ValueGeneratedOnAdd();

                            b1.Property<string>("Code")
                                .HasMaxLength(64);

                            b1.Property<DateTimeOffset?>("CompletedAt")
                                .HasColumnType("timestamptz");

                            b1.Property<string>("Message")
                                .HasMaxLength(500);

                            b1.Property<string>("Status")
                                .IsRequired();

                            b1.Property<string>("Step")
                                .IsRequired();

                            b1.HasKey("WhatsAppConnectionId", "__synthesizedOrdinal");

                            b1.ToTable("whatsapp_connections");

                            b1
                                .ToJson("onboarding_steps")
                                .HasColumnType("jsonb");

                            b1.WithOwner()
                                .HasForeignKey("WhatsAppConnectionId")
                                .HasConstraintName("fk_whatsapp_connections_whatsapp_connections_whats_app_connectio");
                        });

                    b.Navigation("OnboardingSteps");
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.Campaign", b =>
                {
                    b.Navigation("Runs");
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.CampaignRun", b =>
                {
                    b.Navigation("Messages");
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.Contact", b =>
                {
                    b.Navigation("GroupMemberships");

                    b.Navigation("TagAssignments");
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.ContactGroup", b =>
                {
                    b.Navigation("Members");
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.ContactImportBatch", b =>
                {
                    b.Navigation("Exports");

                    b.Navigation("Rows");
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.ContactTag", b =>
                {
                    b.Navigation("Assignments");
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.Conversation", b =>
                {
                    b.Navigation("Messages");
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.Role", b =>
                {
                    b.Navigation("UserRoles");
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.Tenant", b =>
                {
                    b.Navigation("Users");
                });

            modelBuilder.Entity("Marketing.DataAccess.Entities.User", b =>
                {
                    b.Navigation("PermissionOverrides");

                    b.Navigation("RefreshTokens");

                    b.Navigation("UserRoles");
                });
#pragma warning restore 612, 618
        }
    }
}
